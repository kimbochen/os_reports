...
bool
Directory::Add(char *name, int newSector, int isdirectory)
{ 
    if (FindIndex(name) != -1)
	return FALSE;
    for (int i = 0; i < tableSize; i++){
...
{ 
    ...
    // MP4I
    if(table[i].isdir)// don't remove directory
        return FALSE;
    else
        table[i].inUse = FALSE;
    return TRUE;	
...
Directory::RecursiveList()
{
    for(int i = 0; i < tableSize; i++){
        if(table[i].inUse){ 
            ...
            // recursive, be careful that the directory is a file, need to open before use
            if(table[i].isdir){
...
{ 
	...
	if (table[i].inUse) {
        // MP4I
	    printf("Name: %s, Sector: %d, IsDirectory: %d\n", table[i].name, table[i].sector, table[i].isdir);
...
// 	Return whether the name is directory
//----------------------------------------------------------------------
bool
