...
Machine::OneInstruction(Instruction *instr)
{
    ...
    // Fetch instruction 
    if (!ReadMem(registers[PCReg], 4, &raw)) // Fetch register[PCReg] 4 bytes into raw
	return;			// exception occurred
    instr->value = raw;
    instr->Decode(); // Decode instruction value to system call
    ...
    int pcAfter = registers[NextPCReg] + 4; // Point to next instruction
	...
	break;
      ...
      case OP_DIV:
	if (registers[instr->rt] == 0) { // Divide by zero
	    registers[LoReg] = 0;
	    registers[HiReg] = 0;
	} else {
	...
	break;
      ...
      case OP_DIVU: // Unsigned division
	...
	// The only difference between this code and the BIG ENDIAN code
        ...
        // is an arbitrary) This is the whole purpose of LWL and LWR etc.
...
Instruction::Decode()
{
    ...
    opCode = opPtr->opCode;
    if (opPtr->format == IFMT) { // I-type Instruction
	extra = value & 0xffff;
	if (extra & 0x8000) {
    	   extra |= 0xffff0000;
	}
    } else if (opPtr->format == RFMT) { // R-type Instruction
	extra = (value >> 6) & 0x1f;
    } else {
    ...
    } else if (opCode == BCOND) { // Branch condition
	int i = value & 0x1f0000;

	if (i == 0) {
    	    opCode = OP_BLTZ; // Branch if less than zero
	} else if (i == 0x10000) {
    	    opCode = OP_BGEZ; // Branch if greater or equal to zero
	} else if (i == 0x100000) {
    	    opCode = OP_BLTZAL; // Branch if less than zero and link
	} else if (i == 0x110000) {
    	    opCode = OP_BGEZAL; // Branch if greater or equal to zero and link
	} else {
    	    opCode = OP_UNIMP; // Unimplemented
	}
    }
}
