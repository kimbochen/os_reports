...
//  The OpenAFile function is used for kernel open system call
	...
	OpenFileId EmptyPosition = -1;
	for(int i = 0; i < 20; i++)//find empty position
		if(fileDescriptorTable[i] == NULL){
			EmptyPosition = i;
			break;
		}
	if(EmptyPosition == -1) return -1;//file table is full
	fileDescriptorTable[EmptyPosition] = Open(name);
	if(fileDescriptorTable[EmptyPosition] == NULL) return -1;
