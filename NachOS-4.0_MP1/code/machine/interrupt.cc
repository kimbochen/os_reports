...
// check any pending interrupts are now ready to fire
	...
	kernel->currentThread->Yield(); // Do context switch
	status = oldStatus;
    }
}
...
Interrupt::CheckIfDue(bool advanceClock)
{
    ...
    next = pending->Front(); // pending queue store interrupt
			     // ready queue store ready thread

    if (next->when > stats->totalTicks) { // the interrupt in pending queue will occur in future, check whether ready queue is empty or not
        if (!advanceClock) {		// not time yet and ready queue is not empty
            return FALSE;
        }
        else { // ready queue is empty, do future interrupt
	    stats->idleTicks += (next->when - stats->totalTicks); // advance the clock to next interrupt
    ...
    inHandler = TRUE; // in interrupt handler
    do {
        next = pending->RemoveFront();    // pull interrupt off list (pop pending queue)
...
// keep dealing with interrupts until pending queue empty or the first pending queue task should not be done yet(scheduled in future)
    inHandler = FALSE;
    return TRUE;
}
