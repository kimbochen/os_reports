...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
            ...
            debugUserProg = TRUE;
		} else if (strcmp(argv[i], "-e") == 0) {// execute specific file
        	execfile[++execfileNum]= argv[++i];
            execfile_priority[execfileNum] = 0;
			cout << execfile[execfileNum] << "\n";
		} else if (strcmp(argv[i], "-ep") == 0){ // add argument -ep
            execfile[++execfileNum] = argv[++i];
            execfile_priority[execfileNum] = atoi(argv[++i]);
            cout << execfile[execfileNum] << " with priority " << execfile_priority[execfileNum] << "\n";
...
Kernel::Initialize()
{
    ...
    currentThread = new Thread("main", threadNum++, 149); // Thread("main", threadNum++, priority(149))	highest priority	
...
void Kernel::ExecAll()
// Called from threads/main.cc, line 288. Nachos ends after this function returns.
{
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i], execfile_priority[i]);
	}
	currentThread->Finish();// After allocate all needed space for all thread, we start execute all file and delete main thread
    //Kernel::Exec();	
}
...
{
	t[threadNum] = new Thread(name, threadNum, priority);// Thread(name, threadNum, priority);
