...
{
    name = debugName;
    value = initialValue; // Number of spaces for threads available
...
Semaphore::P()
{
    ...
    currentThread->setT( kernel->stats->totalTicks - currentThread->getenter_running_tick() );
    DEBUG(dbgSchedule, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << currentThread->getID()
          << "] update approximate burst time, from: [" << currentThread->gett() << "], add ["
          << currentThread->getT() << "], to [" << ((currentThread->gett() + currentThread->getT())*0.5)  << "]");
    currentThread->sett( 0.5 * (currentThread->gett() + currentThread->getT()));
    //currentThread->T += (kernel->stats->totalTicks - currentThread->enter_running_tick);
    //currentThread->t = (currentThread->t + currentThread->T) * 0.5;

	currentThread->Sleep(FALSE);
    } 
    value--; 			// semaphore available, consume its value. (Because value > 0)
