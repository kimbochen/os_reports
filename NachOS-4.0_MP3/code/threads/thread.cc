...
Thread::Thread(char* threadName, int threadID, int _priority)
{
    ...
    waiting_tick = 0; // Waiting tick for aging
    T = 0; // CPU burst time
    t = 0; // Approximate waiting time
    enter_running_tick = 0;
}
...
Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    oldLevel = interrupt->SetLevel(IntOff);// Return value of old interrupt level
...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) {
        this->T += (kernel->stats->totalTicks - this->enter_running_tick);
...
{
    stack = (int *) AllocBoundedArray(StackSize * sizeof(int));// allocate a memory protected array to avoid illegal reference off the end of the array
